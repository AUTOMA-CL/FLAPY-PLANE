...
export default function GamePage() {
  ...
  return (
    <main className="w-full h-screen relative bg-black overflow-hidden">
      ...
      {/* Información adicional del usuario (móvil) */}
      {currentUser && (
        <div className="absolute bottom-4 right-4 bg-black bg-opacity-50 rounded-lg p-2 text-white text-xs">
          <div>Mejor: {currentUser.bestScore || 0}</div>
          <div>Juegos: {currentUser.totalGames || 0}</div>
        </div>
      )}

      {/* Meta tag para orientación móvil */}
      <div className="absolute bottom-2 left-1/2 transform -translate-x-1/2 text-white text-xs opacity-50 text-center hidden sm:block">
